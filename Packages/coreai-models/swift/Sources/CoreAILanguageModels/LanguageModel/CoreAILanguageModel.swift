...
public struct CoreAILanguageModel: LanguageModel {
    ...
    public struct CoreAIExecutor: LanguageModelExecutor {
        ...
        private func respondVanilla(
            ...
            // NOTE (OnDeviceLAS patch): do not call `.updateUsage(...)`.
            // iOS 27.0 beta builds such as 24A5390f ship a FoundationModels
            // binary that does not export
            // `LanguageModelExecutorGenerationChannel.Response.Action.updateUsage`,
            // while Xcode 27's SDK TBD still lists it. Referencing the symbol
            // aborts the process at dyld load time (EXC_CRASH / SIGABRT,
            // "Symbol not found") before UI can appear. Token accounting is
            // optional for our OpenAI-compatible facade.
